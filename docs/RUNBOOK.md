...
1. Launch the app.
2. Confirm `Í∏∞Í∏∞ ÏïàÏóê Í∏∞ÏñµÌïòÍ∏∞` is visible.
3. Confirm local memory defaults off.
4. Turn local memory on.
5. Select a role such as `ÏΩîÏπò`.
6. Enter a profile name, one person memory, and one todo memory.
7. Tap `Í∏∞Ïñµ Ï†ÄÏû•ÌïòÍ∏∞`.
8. Confirm `ÎÇ¥ Í∏∞Ïñµ` shows role, profile, `Ìï®Íªò ÏïåÏïÑÍ∞ÄÎäî Îã®Í≥Ñ`, `Í∏∞ÏñµÌï† ÏÇ¨Îûå`, and `ÎÇ¥Ïùº Ìï† Ïùº`.
9. Force-stop/relaunch or restart the app.
10. Confirm approved local memory is restored.
11. Tap `Ï†ÄÏû•Îêú Í∏∞Ïñµ Î™®Îëê ÏßÄÏö∞Í∏∞`.
...
3. Role selection for `ÏπúÍµ¨`, `Ïó∞Ïù∏`, `Í∞ÄÏ°±`, `Î∂ÄÎ™®`, `ÏΩîÏπò`, `ÏÑ†ÏÉùÎãò`, `Í≤ΩÏ≤≠Ïûê`, and `ÏÇ¨Ïö©Ïûê ÏßÄÏ†ï`.
...
2. Confirm `Ïò§ÎäòÏùò Ïù∏ÏÇ¨Ïù¥Ìä∏` appears in the local memory area.
...
5. `ÎÇ¥ Í∏∞Ïñµ` or equivalent stored-information area.
...
2. Confirm `Ïò§Îäò ÏãúÏûëÌïòÍ∏∞` appears even on the fallback state.
...
8. Confirm `Keep for morning -> ÎÇ¥Ïùº ÏãúÏûë Î©îÎ™®` still works as a separate session-only flow.
...
4. Keep one reflection so «œ∑Á ±‚∑œ appears in ≥ª ±‚æÔ ∞¸∏Æ.
5. Confirm ≥ª º“∞≥, ±‚æÔ«“ ªÁ∂˜, ≥ª¿œ «“ ¿œ, and «œ∑Á ±‚∑œ each show a category count.
...
8. Confirm «‘≤≤ æÀæ∆∞°¥¬ ¥‹∞Ë, ø¿¥√¿« ¿ŒªÁ¿Ã∆Æ, and ø¿¥√ Ω√¿€«œ±‚ continue rendering from the updated snapshot.
9. Turn consent OFF and confirm ≥ª ±‚æÔ ∞¸∏Æ hides stored category details behind the fallback management message.
10. Tap ¿˙¿Âµ» ±‚æÔ ∏µŒ ¡ˆøÏ±‚ and confirm the app returns to the empty local-memory summary state.
...
2. Turn ±‚±‚ æ»ø° ±‚æÔ«œ±‚ ON so role-based local-memory UI remains active.
3. Select ƒ£±∏, ƒ⁄ƒ°, ∞Ê√ª¿⁄, and º±ª˝¥‘ in turn and confirm the current-role context line changes in Korean.
...
5. Confirm ø¿¥√¿« ¿ŒªÁ¿Ã∆Æ changes its ¡˙πÆ/¿€¿∫ πÃº« tone by role without claiming certainty or diagnosis.
6. Confirm ø¿¥√ Ω√¿€«œ±‚ changes its ø¿¥√¿« ¡˙πÆ/√π π¯¬∞ ¿€¿∫ «‡µø/¿¿ø¯ πÆ±∏ by role.
...
2. Confirm the visible section order reads naturally as:
   ...
   - ≥ª ±‚æÔ ∞¸∏Æ
   - ¿¸√º √ ±‚»≠
3. Confirm ø¿¥√¿« ¿ŒªÁ¿Ã∆Æ appears before ø¿¥√ Ω√¿€«œ±‚.
4. Confirm ≥ª ±‚æÔ ∞¸∏Æ stays reachable through an explicit expand/collapse control and does not crowd the main entry flow.
5. Confirm ¿˙¿Âµ» ±‚æÔ ∏µŒ ¡ˆøÏ±‚ remains visible and reachable outside the collapsed memory-management details.
