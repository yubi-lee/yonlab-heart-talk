...
3. Role selection for `ì¹œêµ¬`, `ì—°ì¸`, `ê°€ì¡±`, `ë¶€ëª¨`, `ì½”ì¹˜`, `ì„ ìƒë‹˜`, `ê²½ì²­ì`, and `ì‚¬ìš©ì ì§€ì •`.
...
2. Confirm `ì˜¤ëŠ˜ì˜ ì¸ì‚¬ì´íŠ¸` appears in the local memory area.
...
5. `ë‚´ ê¸°ì–µ` or equivalent stored-information area.
...
2. Confirm `ì˜¤ëŠ˜ ì‹œì‘í•˜ê¸°` appears even on the fallback state.
...
8. Confirm `Keep for morning -> ë‚´ì¼ ì‹œì‘ ë©”ëª¨` still works as a separate session-only flow.
...
10. Tap ÀúÀåµÈ ±â¾ï ¸ğµÎ Áö¿ì±â and confirm the app returns to the empty local-memory summary state.
...
3. Select Ä£±¸, ÄÚÄ¡, °æÃ»ÀÚ, and ¼±»ı´Ô in turn and confirm the current-role context line changes in Korean.
...
5. Confirm ÀúÀåµÈ ±â¾ï ¸ğµÎ Áö¿ì±â remains visible and reachable outside the collapsed memory-management details.
...
2. Tap `100ì¼ ì„±ì¥ ì²´í—˜í•˜ê¸°`.
3. Choose a Korean scene preset such as `ì°½ì—…ì ë°”ìœ í•˜ë£¨`, `íšŒì‚¬ ì—…ë¬´ ìŠ¤íŠ¸ë ˆìŠ¤`, or `ë²ˆì•„ì›ƒ íšŒë³µ`.
...
6. Tap `ì‹œë®¬ë ˆì´ì…˜ ê¸°ì–µ ì§€ìš°ê¸°` and confirm the demo returns to fallback state without clearing the real local memory snapshot.
7. If the real local memory should also be cleared, use the existing `ì €ì¥ëœ ê¸°ì–µ ëª¨ë‘ ì§€ìš°ê¸°` control in the normal memory flow.
...
3. Confirm the copy explains local-only storage, role selection, local insights, morning brief, synthetic `100ì¼ ì„±ì¥ ì²´í—˜`, and memory deletion.
4. Tap `ì‹œì‘í•˜ê¸°` or `ì´í•´í–ˆì–´ìš”` and confirm the main screen becomes visible.
5. Tap `ë„ì›€ë§ ë‹¤ì‹œ ë³´ê¸°` from the app bar and confirm the onboarding guide can be reopened.
