...
| DEC-015 | 2026-06-30 | Add snapshot-level local memory management as an MVP inside the existing daily reflection screen. | Alternatives: full-screen CRUD flow, new local database layer, schema migration with per-item storage, reset-only control. | The current local-memory feature needs finer user control, but the safest small step is to edit and delete specific items from the existing consent-filtered snapshot. | Adds an application-layer memory management service and scoped widget actions without changing storage keys, dependencies, or platform settings. | Accepted |
