...
bool _containsSensitivePattern(String value) {
  return RegExp(r'(\b\d{2,3}-\d{3,4}-\d{4}\b|@|https?://|www\.)').hasMatch(value);
}
...
void main() {
  ...
  test('generates a 100-day synthetic snapshot with high growth', () {
    ...
    expect(snapshot.recurringKeywords.values.where((count) => count >= 2), isNotEmpty);
  ...
  test('creates different scene specific signals and summaries', () {
    final founder = simulationService.generate(
      scene: syntheticGrowthScenePresets
          .firstWhere((preset) => preset.id == 'founder-busy-day'),
      generatedAt: DateTime.utc(2026, 6, 30),
    );
    final listener = simulationService.generate(
      scene: syntheticGrowthScenePresets
          .firstWhere((preset) => preset.id == 'quiet-listening'),
      generatedAt: DateTime.utc(2026, 6, 30),
    );
  ...
  test('generated snapshot produces personalized insight and morning brief', () {
    final session = simulationService.generate(
      scene: syntheticGrowthScenePresets
          .firstWhere((preset) => preset.id == 'goal-coaching'),
      generatedAt: DateTime.utc(2026, 6, 30),
    );
    final snapshot = session.snapshot;
    final growthState = growthCalculator.calculate(snapshot);
  ...
  test('repository save load and clear keeps simulation separate', () async {
    ...
    final repository = SharedPreferencesSyntheticGrowthSimulationRepository(prefs);

    final session = simulationService.generate(
      scene: syntheticGrowthScenePresets
          .firstWhere((preset) => preset.id == 'weekend-recovery'),
      generatedAt: DateTime.utc(2026, 6, 30),
    );
    ...
    expect(loaded.snapshot.dailyEntries.length, session.snapshot.dailyEntries.length);
